...
app.post("/toggle-motor", (req, res) => {
  ...
  if (motorStatus) res.send("Motor is running");
  else res.send("Motor is stopped");
});
...
app.post("/schedule", (req, res) => {
  ...
  if (scheduleDate < now) {
    return res
      .status(400)
      .json({ message: "Cannot add a schedule in the past." });
  }
...
app.get("/allSchedules", (req, res) => {
  const schdles = schedules.map((schedule) => ({
    date: `${(schedule.month + 1).toString().padStart(2, "0")}.${schedule.day
      .toString()
      .padStart(2, "0")}`,
    time: `${schedule.hour.toString().padStart(2, "0")}:${schedule.minute
      .toString()
      .padStart(2, "0")}`,
    pet: "placeholder pet",
